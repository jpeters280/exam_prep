...
class UserModel(Model):
    ...
    def create_user(self, info):
        ...
        if errors:
            ...
            data2 =  {'name':info['name'], 'alias':info['alias'], 'birthday':info['birthday'], 'email':info['email'],'pw_hash':pw_hash}
            query_insert = "INSERT into users (name, alias, birthday, email, pw_hash, created_at, updated_at) Values (:name, :alias, :birthday, :email, :pw_hash, NOW(), NOW())"
    ...
    def showname(self,id):
        query = "SELECT alias from users where users.id = :id limit 1"
        data = {"id":id}
        return self.db.query_db(query, data)

    def showquote(self,id):
        query = "SELECT quote_by, content from quotes where user_id = :id"
        data = {"id":id}
        return self.db.query_db(query, data)

    def countquote(self, id):
        query = 'SELECT count(content) from quotes where user_id = :id'
        data = {'id':id}
        return self.db.query_db(query, data)
